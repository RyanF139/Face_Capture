...
from video_worker_v2 import WEBHOOK_API
...
# Video tidak di-loop — proses sekali lalu render + upload
VIDEO_LOOP         = os.getenv("VIDEO_LOOP", "false").lower() == "true"

# ── S3 ────────────────────────────────────────────────────
# S3_ENDPOINT_URL : kosong = AWS S3, isi = MinIO / Cloudflare R2 / dll
# S3_PUBLIC_BASE  : opsional, jika pakai CDN / bucket public
#                   contoh: https://cdn.example.com
#                   hasil URL = S3_PUBLIC_BASE/{s3_key}
#                   jika kosong → pakai presigned URL (TTL 7 hari)
...
class S3Uploader:
    """
    Upload file ke S3-compatible storage (AWS S3 / MinIO / R2).

    Best practices:
    - boto3 client dibuat sekali, thread-safe untuk concurrent upload
    - multipart otomatis untuk file > 8MB (handled by boto3)
    - Kembalikan public URL atau presigned URL (TTL 7 hari)
    """
    ...
    def upload_file(self, local_path: str, s3_key: str,
        ...
        """
        Upload file ke S3.
        Return URL (public CDN atau presigned).
        """
...
# Inisialisasi singleton, None jika S3 tidak aktif
s3_uploader = S3Uploader() if S3_ENABLED else None
...
class FrameVideoWriter:
    """
    Kumpulkan annotated frame selama worker berjalan,
    render menjadi MP4 saat worker selesai.

    Strategi penyimpanan ke disk (bukan RAM):
    - Aman untuk video panjang / banyak worker paralel
    - Render via FFmpeg image2 muxer (lebih baik dari cv2.VideoWriter)
    - Cleanup temp folder otomatis setelah render
    """
    ...
    def write(self, frame: np.ndarray):
        """Simpan satu frame annotasi (dipanggil tiap frame di worker loop)."""
    ...
    def render(self) -> str | None:
        """
        Render semua frame → MP4.
        Return path MP4 lokal, atau None jika tidak ada frame / gagal.
        """
        if self.frame_idx == 0:
        ...
        cmd = [
            ...
            "-movflags", "+faststart",   # MP4 bisa di-stream sebelum download penuh
            out_path
        ]
...
def webhook_worker():
    """Thread pool untuk kirim face detection event (non-blocking)."""
    while True:
        ...
        try:
            ...
            print(f"[WEBHOOK] Face event: {face_name}")
...
class VideoWorker:
    ...
    def destroy(self, reason: str = "done"):
        ...
        2. Release capture
        3. Cleanup preview
        4. Spawn thread finalize (render → S3 → webhook)
           → non-blocking agar tidak freeze video_manager
        """
        self.running = False
    ...
    def _finalize(self, reason: str):
        """
        Pipeline finalize (di thread terpisah):
        render MP4 → upload S3 → kirim webhook done
        """
        s3_url     = None
        local_path = None
    ...
    def _send_done_webhook(self, reason: str, s3_url, local_path):
        ...
        Payload:
        {
          ...
          "timestamp":  "2026-...",
          "result": {
            "s3_url":     "https://...",   # null jika upload gagal / S3 off
            "local_path": "/path/to/result.mp4",
            "uploaded":   true | false
          }
        }
        """
        ...
        print(f"[WEBHOOK DONE] {json.dumps(payload, indent=2)}")

        try:
            resp = requests.post(WEBHOOK_API+"/webhook/receive-status-video", json=payload, timeout=15)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if not ret:
                ...
                if self.bad >= self.max_bad:
                    ...
                    if VIDEO_LOOP:
                        ...
                        # Video habis → trigger finalize pipeline
            ...
            # ── Simpan frame annotasi untuk result MP4 ─────
