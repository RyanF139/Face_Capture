...
class VideoWorker:
    ...
    def _open_cap(self, url: str) -> cv2.VideoCapture:
        ...
        # File lokal atau path debug
        if os.path.isfile(url):
            cap = cv2.VideoCapture(url)
        else:
            # HTTP/HTTPS MP4 atau YouTube direct stream
            ...
            if not ret:
                ...
                if self.bad >= self.max_bad:
                    ...
                    if VIDEO_LOOP:
                        ...
                        print(f"[{self.video_id}] Worker berhenti.")
                        self.running = False
        ...
        with camera_lock:
            ...
            for vid in list(existing):
                if vid not in current_ids:
                    ...
                    active_cameras[vid].running = False
                    del active_cameras[vid]
                    preview_frames.pop(vid, None)
